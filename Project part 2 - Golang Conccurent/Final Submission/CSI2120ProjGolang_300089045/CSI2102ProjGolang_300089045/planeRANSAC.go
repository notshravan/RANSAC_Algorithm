...
func main() {
	...
	for i := 0; i < 3; i++ {
		...
		planeStream := PlaneEstimator(wg, stopChan,
			TakeN(wg, stopChan,
				TripletPointGen(GenRandomPoints(points, RandomPoints, stopChan, wg), stopChan, wg), numIterations))
		...
		saveXYZ(saveFilename, GetSupportingPoints(bestPlaneSupport.plane, points, distanceThreshold))
